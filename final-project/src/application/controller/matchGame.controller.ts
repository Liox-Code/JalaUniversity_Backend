...
class MatchGameController extends BaseHttpController {
  ...
  @httpGet('/create')
  public async create (@queryParam('size') size: number, @queryParam('refreshTimeRare') refreshTimeRare: number, req: Request, res: Response, next: NextFunction) {
  ...
  public async matchCurrentFrame (@queryParam('matchId') matchId:number, @queryParam('status') status: EMatchGameState, req: Request, res: Response, next: NextFunction) {
